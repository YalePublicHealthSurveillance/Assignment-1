...
MAC USERS: Change the directory as needed to point to where the .csv file is saved. Or just go to Session/Set Working Directory/To Source File Location

```{r readscsv}
d1 <- read.csv('chile_hiv.csv')
```
...
```{r}
d5 <- d1[d1$date<as.Date('2009-01-01'),] %>%
  group_by(week.date) %>%
  summarize('hiv'=sum(hiv)) %>%
  ungroup()
d5 <- as.data.frame(d5)
```
...
p1 <- ggplot(d5, aes(x=week.date, y=hiv)) +
  geom_line() +
  ylab('Cases of HIV') +
  theme_classic()
p1
...
```{r}
glrpois_App(ds=d5, datevar='week.date', casevar='hiv', n.weeks.train=53)
...
e1<-readRDS('ch.birth.defects.rds')
